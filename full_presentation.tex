...
\note[item]{
        Very easy over the reals, hard otherwise
        \item Solvable? How many solutions?
        \item Undecidable in 11 variables already
}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Given a rational point, find all geometrically
        \item Hasse principle allows us to test if a rational point exists
    \end{itemize}
    \note[item]{
        Take the rational point, draw a line
        \item Hasse = Local to Global: Solvable in rational iff in reals and p-adic for every p
    }
...
\begin{frame}
    ...
    \end{center}
    \note[item]{
        Local to global fails
        \item Originated in computation of arc length of ellipse
    }
\end{frame}
...
\begin{frame}
    ...
    \end{definition}
    \pause
    Informally, we can add, subtract, multiply and divide.
\end{frame}
...
\begin{frame}
    ...
    \note[item]{
        If two fields have different char, no map between them (apart 0)
...
\begin{frame}
    ...
    \end{itemize}
    \note[item]{
        Closures are always infinite
        \item Can approximate $\overbar{\FF}$ with $\FF_{p^n!}$
        \item Base of Galois theory
    }
\end{frame}
...
\begin{frame}
    ...
    \note[item]{
        Weierstrass most common (academically)
        \item Other models exist
        \item Montgomery curves (x-only)
        \item Edwards curves (Complete addition formula)
        \item Legendre curves
    }
\end{frame}
...
\begin{frame}
    ...
    \end{definition}
    \pause
    Mathematicians often interested with $E(\QQ) \subseteq E(\mathbb{R}) \subseteq E(\mathbb{C})$ \\
    We mostly consider the finite case.
    \note[item]{
        Projective closure
        \item Point at infinity correspond to $(0: 1: 0)$
    }
\end{frame}
...
\begin{frame}
  ...
  \end{tikzpicture}
    ...
    \note[item]{
        Top are non singular
        \item First singular has cusp
        \item Second has a node
    }
\end{frame}
...
\begin{frame}
   ...
   \end{definition}
   \pause
    Alternatively, let $E: y^2 = f(x)$, and let $x_1, x_2, x_3$ be $f$'s roots. 
...
\begin{frame}
    ...
    \end{theorem}
    \pause
    Can be efficiently found using Schoof's algorithm in $O((\log q)^8)$.
    \note[item]{
        Each value of $x$ yields at most two of $y$
        \item a random element has 50\% of being quadratic residue
    }
\end{frame}
...
\begin{frame}
    ...
    \onslide<5->{
    \[ \mathrm{DDH} \leq_R \mathrm{CDH} \leq_R \mathrm{DLP} \]}

    \onslide<6->{
    \textbf{Representation matters}! $\ZZ_{p-1} \cong \ZZ^*_p$ as groups \\ 
    Discrete logarithm trivial in one, assumed hard in other. }
    \note[item]{
        In fact in some groups CDH and DLP are equivalent
        \item Check $e(g^x, g^y) = e(g, g^z)$
    }
\end{frame}
...
\begin{frame}
    ...
    \note[item]{
        128 bits
        \item RSA public key ops are faster, private ops slower
    }
\end{frame}
...
\begin{frame}
    ...
    \[ K = [x]Q_B = [x][y]P = [y][x]P = [y]Q_A = K \]
    \note[item]{
        Need to check received points are on curve
        \item Invalid points attacks
    }
\end{frame}
...
\begin{frame}
    ...
    \end{itemize}
    \note[item]{
        Embedding degree -> MOV algorithm
    }
\end{frame}
...
\begin{frame}
    ...
    \end{center}
    \note[item]{
        In practice, to detect cycle step one iterator by one and one by two
        \item Not parallelizable, use vOW (more memory)
    }
\end{frame}
...
\begin{frame}
    ...
    \note[item]{
        ...
        \item The $\gcd$ condition in pratice almost always true
    }
\end{frame}
...
\begin{frame}
    ...
    \begin{definition}
        ...
        \end{itemize}
        \note[item]{
            The alternating not strictly in definition
            \item Generalised with three groups
        }
    \end{definition}
\end{frame}
...
\begin{frame}
    ...
    \note[item]{
        Not every curve has distorsion maps
    }
\end{frame}
...
\begin{frame}
    ...
    \pause
    Correctness by:
    \[ e(\sigma, P) = e([x]Q, P) = e(Q, [x]P) = e(H(m), [x]P) \]
...
\begin{frame}
    ...
    \end{definition}
    \pause
    For example, the curves $y^2 = x^3 + x$ and $y^2 = x^3 - 3x + 3$ are isogenous over $\FF_{71}$ via the isogeny that maps $(x, y) \mapsto$ 
    \[ \left(\frac{x^3 - 4 x^2 + 30 x -12}{(x - 2)^2}, y\cdot\frac{x^3 - 6x^2 -14x + 35}{(x - 2)^3} \right)\]
\end{frame}
...
\begin{frame}
    ...
    If it is Frobenius + isomorphism it is purely inseparable. \\
    \pause
    We are mostly  concerned with the separable case.
    \note[item]{
        Frobenius are very important
        \item In our case they are a bit of a nuisance for a theorem later on
    }
\end{frame}
...
\begin{frame}
    ...
    \end{theorem}
    \pause
       Let $E/k$, with $k$ a finite field. \\ 
       For any subgroup $H \leq E$ we can find an 
       isogeny of kernel $H$ in $\Theta(\#H)$ by Velu's formulas. \\ 
       We denote the target of that isogeny by $E/H$
...
\begin{frame}
    ...
    \end{itemize}
    \note[item]{
        Endomorphism ring is a order in a quaternion algebra
    }
...
\begin{frame}
    ...
    \begin{definition}
        ...
        \begin{itemize}
            \item<2-> Vertices: Supersingular Elliptic curves over $K$
...
Details will follow
\note[item]{
    Alice computes the left to right ones
    \item Bob computes the down arrows
}
\end{frame}
...
\begin{frame}
    ...
    \pause a supersingular starting curve $E/\FF_{p^2}$,
...
\begin{frame}
    ...
    supersingular and over $\FF_{p^2}$.
